...
export class DatasetsRecordComponent {
    ...
    public addCSWRecord(cswRecord: CSWRecordModel): void {
        let onlineResources: OnlineResourceModel[] = cswRecord.onlineResources;
        if (onlineResources && onlineResources.length > 0) {
            const wmsResourceIndex = onlineResources.findIndex(r => r.type === 'WMS');
            if (wmsResourceIndex !== -1 && this.isGetCapabilitiesUrl(onlineResources[wmsResourceIndex].url, 'wms')) {
                let wcsResourceIndex = onlineResources.findIndex(r => r.type === 'WCS');
                if (wcsResourceIndex !== -1 && this.isGetCapabilitiesUrl(onlineResources[wcsResourceIndex].url, 'wcs')) {
                    onlineResources[wmsResourceIndex].url = onlineResources[wmsResourceIndex].url.substring(0, onlineResources[wmsResourceIndex].url.indexOf('?') + 1) + "service=WMS";
                    onlineResources[wcsResourceIndex].url = onlineResources[wcsResourceIndex].url.substring(0, onlineResources[wcsResourceIndex].url.indexOf('?') + 1) + "service=WCS";
                }
            }
        }
